...
function totalItemEmEstoque(array){
  const qtdEstoque = array.map(produto => produto.qtdEstoque)
  const resultado = qtdEstoque.reduce((acumulador , atual) => acumulador + atual )
  return console.log(`Quantidade De Produtos Em Estoque : ${resultado} Produtos`)
}
function totalItemEmDestaque(array){
  const itemDestaque = array.filter(produto => produto.emDestaque == 'sim').length
  console.log(`Total de itens em Destaque : ${itemDestaque}`)
  return itemDestaque
}
function totalItemDisponivel(array){
  const itemDisponivel = array.filter(produto => produto.disponivel == 'sim').length
  console.log(`Total de itens Disponiveis : ${itemDisponivel}`)
  return itemDisponivel
}

function disponivelEmDestaque(array){
  const itemEmDestaque = array.filter(produto => produto.disponivel == 'sim' && 
  produto.emDestaque == 'sim').length 
  console.log(`Total de itens Disponiveis em Destaque : ${itemEmDestaque}`)
}
function inventarioLoja(array){
  const item = array.length
  console.log(`Total de Produtos no inventario da Loja : ${item}`)
}
function produtoMaisCaro(array){
  const produtoMaisCaro = array.reduce((acumulador , atual)=> acumulador.preco >= atual.preco ? 
  acumulador : atual , 0)
  return console.log(`O Produto Mais Caro é ${produtoMaisCaro.descricao}
  preço: ${produtoMaisCaro.preco.toFixed(2).toString().replace(".",",")} Reais
  Departamento:${produtoMaisCaro.departamento.nomeDepto}`)
}
function produtoMaisBarato(array){
  const produtoMaisBarato = array.reduce((acumulador , atual)=> acumulador.preco <= atual.preco ?
  acumulador : atual)
  return console.log(`O Produto Mais Barato da Loja é ${produtoMaisBarato.descricao}
  Preço: ${produtoMaisBarato.preco.toFixed(2).toString().replace(".",",")}
  Departamento:${produtoMaisBarato.departamento.nomeDepto}`)
}

function produtoMaiorValorEmEstoque(array){
  const filtrarEstoqueDisponivel = array.filter(produto => produto.qtdEstoque >= 1)
  const maisCaroEstoqueDisponivel = filtrarEstoqueDisponivel.reduce((acumulador, atual) =>
  acumulador.preco >= atual.preco ? acumulador : atual )
  return console.log(`Produto com maior valor Disponivel:${maisCaroEstoqueDisponivel.descricao}
  valor do produto = ${maisCaroEstoqueDisponivel.preco.toFixed(2).toString().replace(",",".")} R$`)
}

function produtoMenorValorEmEstoque(array){
  const filtrarEstoqueDisponivel = array.filter(produto => produto.qtdEstoque >= 1)
  const produtoMenorValorDisponivel = filtrarEstoqueDisponivel.reduce((acumulador , atual) =>
  acumulador.preco <= atual.preco ? acumulador : atual)
  return console.log(`Produto com menor valor Disponivel:${produtoMenorValorDisponivel.descricao}
  valor do produto de MenorValor:
  ${produtoMenorValorDisponivel.preco.toFixed(2).toString().replace(",",".")} R$`)
}


function valorTicketMedio(array){ 
  const objLoja = array.map(produto => produto.preco)
  const valorTotal = objLoja.reduce((acumulador , atual) => acumulador+= atual)
  const mediaTicket = valorTotal / array.length
...
totalItemDisponivel(listaProdutos)
disponivelEmDestaque(listaProdutos)
inventarioLoja(listaProdutos)
